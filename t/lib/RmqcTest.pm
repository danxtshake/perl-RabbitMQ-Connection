...
our @EXPORT_OK = qw(skip_unless_broker conn ctl have_ctl uname topology);
use Exporter 'import';

sub skip_unless_broker {
    plan skip_all => 'set TEST_HOST (and TEST_PORT/TEST_USER/TEST_PASS/TEST_VHOST) to a test broker'
        unless $ENV{TEST_HOST};
}

sub have_ctl { defined $ENV{TEST_CTL} && length $ENV{TEST_CTL} }
