# unknown outcomes: under a broker resource alarm settlement stalls, so the
# bounded wait returns "timeout"; a broker-initiated close during the wait
# croaks. Both mean "unknown": the caller must discard the connection.
use strict; use warnings;
use lib 't/lib';
use Test::More;
use RmqcTest qw(skip_unless_broker conn ctl have_ctl uname topology);
skip_unless_broker();
plan skip_all => 'set TEST_CTL to a rabbitmqctl command prefix' unless have_ctl();
plan tests => 6;
